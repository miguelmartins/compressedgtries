      ...
      for (i=0; i<num; i++) 
      {
        ...
        if (j+1 == size)
        {
          ...
          if(c->numClaws(v[i]) > 0)
          {
             ...
             int bound = min(remain_size, c->numClaws(v[i]));

             for(int comb = 1; comb <= bound; comb++)
             {
               ext2[next2].label = current[size - 1].label;
               ext2[next2].claw = c->numClaws(v[i]);
               ext2[next2].k = comb;
               next2++;
             }
          } 
...
void Esu::compressedCountSubgraphs(CompressedGraph *g, int k, GraphTree *sg) {
  ...
  sg->zeroFrequency();
   ...
   for (i=0; i<_graph_size; i++)
   {
      ...
      q.claw = 0;
      q.k = 0;
      std::cout << "Ramo do " << i << endl;
      if(c->numClaws(i) > 0)
      {
         int remain_size = _motif_size - 1;
         int bound = min(remain_size, c->numClaws(i));
         candidate cl[bound];
         for(int comb = 1; comb <= bound; comb++)
         {
           cl[comb-1].label = i;
           cl[comb-1].claw = c->numClaws(i);
           cl[comb-1].k = comb;
           next++;
         }
         compressedGo(q, 0, 0, next, cl);
         next = 0;
      }
      else
