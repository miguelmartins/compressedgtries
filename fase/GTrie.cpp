...
void GTrie::printClass(GTrie::labelTrie *node, char* label, int sz, double prob)
{
  if (node->num != 0)
  {
    label[sz] = '\0';
...
void GTrie::listClasses(FILE* out, double prob)
{
  f = out;
  char tmpStr[MAXS];
  tmpStr[0] = '\0';
  GTrie::printClass(labelRoot, tmpStr, 0, prob);
}
